...
const DateSlider: React.FC<DateSliderProps> = ({
        ...
        setTimeout(() => {
            if (flatListRef.current && currentDateIndex >= 0) {
                flatListRef.current.scrollToOffset({
                    offset: currentDateIndex * (DAY_ITEM_WIDTH + ITEM_SPACING) - (width - DAY_ITEM_WIDTH) / 2,
                    animated: true
                });
            }
        }, 100);
    ...
    const renderDateItem = ({ item: date, index }: { item: Date, index: number }) => {
        ...
        return (
            ...
            >
                {isFirstDayOfMonth && (
                    <View style={[styles.monthBadge, { backgroundColor: _isSelected ? colors.primary + '70' : colors.primary + '20' }]}>
                        <Text style={[styles.monthBadgeText, { color: _isSelected ? colors.background : colors.primary }]}>{monthAbbr}</Text>
                    </View>
                )}
